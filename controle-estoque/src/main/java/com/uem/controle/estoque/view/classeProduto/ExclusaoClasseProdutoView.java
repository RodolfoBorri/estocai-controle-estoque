...
public class ExclusaoClasseProdutoView extends ViewBase{
	...
	public static void run() {
		try {
			ExclusaoClasseProdutoView window = new ExclusaoClasseProdutoView();
	...
	public ExclusaoClasseProdutoView() {
	...
	private void montaCabecalhoCompleto() {
		...
		this.add(montaTituloTela("EXCLUSÃO DE CLASSE DE PRODUTO"));		
	}
	...
	private void initialize() {
		...
		frame.addWindowListener(new WindowAdapter() {
			...
			public void windowClosed(WindowEvent arg0) {
				frame.dispose();
				ClasseProdutoView classeProduto = ApplicationContextProvider.getContext().getBean(ClasseProdutoView.class);
				classeProduto.frame.setVisible(true);
			}
		});
		...
		btnProduto_3.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int result = JOptionPane.showConfirmDialog(frame,
			            "Deseja confirmar a exclusão ?", "Confirmação de exclusão",
			            JOptionPane.YES_NO_OPTION);
				
			        if (result == JOptionPane.YES_OPTION) {
			        	erros = classeProdutoController.removeClasseProduto(textFieldCodigo.getText());
			        	
			        	if(erros.equals("")) {
				            JOptionPane.showMessageDialog(null, "Classe de Produto removida com sucesso!", 
				        		  "SUCESSO", JOptionPane.INFORMATION_MESSAGE);
			        	}
			        	else {
			        		JOptionPane.showMessageDialog(null, erros, "ERROS", JOptionPane.WARNING_MESSAGE);
			        	}
			        }
